...
from PySide6.QtGui import QPainter, QPixmap, QColor, QBrush, QPen
...
class Run(QtCore.QObject):
    save_data = QtCore.Signal(str, int, list, float, numpy.ndarray, str, int, int, list)
    ...
    def __init__(self, Device, ui, model):
        super().__init__()
        self.thread1 = None
    ...
    def read_config(self):
        ...
        center_x = config.getfloat('Setup', '玻片扫描中心x')
        center_y = (config.getfloat('Setup', '玻片扫描中心y'))
        ...
        zpos_start = (config.getfloat('Setup', '对焦经验值'))
        multiple = (config.get('Setup', '对焦倍数'))
        ID = (config.get('Setup', '玻片ID'))

        return center_x, center_y, region_w, region_h, calibration_50, focusing_method, zpos_start, multiple, ID

    @QtCore.Slot(str)
    def finish(self, ID):
        # 获取第二层
        Graph2 = Graph()
        code = Graph2.get_IDfromCode(ID + '_50x')
        self.node_info = Graph2.get_node_info(code)
        # 获取拼接图
        path = self.node_info['Data']['拼接图']
        # 读取图像文件并显示在QLabel中
        ...
        pen.setWidth(2)
        painter.setPen(pen)
        points = self.node_info['Data']['推荐视野']
        view_number = self.node_info['Data']['视野长宽']
        for point in points:
            x1, y1 = (point[0] % view_number[0] - 1, point[0] // view_number[1])
            x2, y2 = (point[3] % view_number[0] - 1, point[3] // view_number[1])
        ...
        self.ui.textEdit_infor_only.append('显微镜平台处理完成')
        # 获取结果数据
        mean_wbc = self.node_info['Data']['白细胞平均数']
        median_wbc = self.node_info['Data']['白细胞中位数']
        mean_ec = self.node_info['Data']['上皮细胞平均数']
        median_ec = self.node_info['Data']['上皮细胞中位数']
        self.ui.textEdit_infor_only.append(
            '白细胞平均数：' + str(mean_wbc) + '     ' + '白细胞中位数：' + str(median_wbc) + '     ' +
            '上皮细胞平均数：' + str(mean_ec) + '     ' + '上皮细胞中位数：' + str(median_ec))
    ...
    def updateLabelpoints(self, center_x, center_y, point, wcalibration, hcalibration, region_width, region_height):
        ...
        radius = 2
        painter.drawEllipse(
            int((center_y - 12 - region_height / 2) / 76.2 * 480 + (point[1] * hcalibration) / 76.2 * 480),
            int((center_x - 1 - region_width / 2) / 25.4 * 160 + (point[0] * wcalibration) / 25.4 * 160),
            radius,
            radius)
    ...
    def Focus_on(self, multiple, zpos_start):
        ...
        for i in range(60):
            # 往下
            self.Device.Microscope.navigationController.move_z_to(zpos_start + 0.003 * (i + 1))
    ...
    @QtCore.Slot(str, int, list, float, numpy.ndarray, str, int, int, list)
    def save(self, PP_code, a, Point_XY, Zpos, img_save, ID, numberw, numberh, point_xy_real):
        # 检查文件夹路径是否存在
        path_save = 'pic\\' + ID
        ...
        timesave = get_time()
        # 粘贴拼图
        img_np = cv2.resize(img_save, (320, 320))
        img_pil = Image.fromarray(img_np)
        self.Stitch_pic.paste(img_pil, box=(Point_XY[1] * 320, Point_XY[0] * 320))
        # 保存原图
        bgr_image = cv2.cvtColor(img_save, cv2.COLOR_RGB2BGR)
        self.ImageSaver.enqueue(bgr_image, self.Graph_class, PP_code, ID, Zpos, a, Point_XY, timesave,
                                path_save, self.Stitch_pic, numberw, numberh, self.ok, point_xy_real)

    def start(self):
        self.ImageSaver = ImageSaver.ImageSaver(self.model)
        self.flage_run = True
        self.ui.textEdit_infor_only.append('显微镜平台开始扫描')
        self.ui.textEdit_infor_only.append('请等待...')
        time_start = time.time()  # 开始计时
        center_x, center_y, region_w, region_h, calibration, focusing_method, zpos_start, multiple, ID = self.read_config()
        # 判断倍数确认光源设置
        if multiple == '50x':
            ...
            image = self.get_image_camera50x()
            ...
            image = self.get_image_camera20x()

        points_4, points_xy, x_Start, y_Start, number_x, number_y, h, w = Route.get_route(image, center_x, center_y,
                                                                                          region_w,
        ...
        try:
            ...
            PP_code = data.Save_data_2(self.Graph_class, root_id, ID, multiple, focusing_method,
                                       number_x, number_y)
            self.Stitch_pic = Image.new("RGB", size=(number_x * 320, number_y * 320))
            # 扫描方式
            if focusing_method == '快速扫描二':
                ...
                for Point_XY in points_xy:
                    ...
                    self.save_data.emit(PP_code, a, Point_XY, zpos, img, ID, number_x, number_y, point_xy_real)
                    a = a + 1
                ...
                for Point_XY in points_xy:
                    ...
                    while self.Device.Microscope.microcontroller.is_busy():
                        time.sleep(0.005)
                    zpos, img = self.Focus_on(multiple, zpos_start)
            ...
            self.ui.textEdit_infor_only.append('显微镜平台扫描完成')
            self.ui.textEdit_infor_only.append('显微镜平台扫描费时' + str(time_c))
            self.ui.textEdit_infor_only.append('显微镜平台还未处理完成')
            self.ui.textEdit_infor_only.append('请等待...')
            ...
            self.ui.pushButton_live_close.setEnabled(True)
            self.ui.pushButton_move2view.setEnabled(True)
            self.ui.textEdit_infor_only.append('显微镜平台扫描失败')
