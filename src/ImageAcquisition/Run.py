...
from src.ImageAcquisition import focus
from src.ImageAcquisition import Route
from src.DataSaver import Saverdata
...
class Run(QtCore.QObject):
    ...
    ok = QtCore.Signal(str, numpy.ndarray, str)
    ...
    def __init__(self, Device, ui):
        ...
        self.slide_map = QPixmap("./src/UI/480.png")
    ...
    def finish(self, ID, img, multiple):
        msg = datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '玻片' + ID + '已扫描' + '\n'
        with open("log.txt", "a") as log_file:
            log_file.write(msg)
        # 获取拼接图
        node = self.Saver.DataProcessing.G.get_IDfromCode(ID + '_' + multiple)
    ...
    def Focus_on(self, multiple, zpos_start):
        ...
        try:
            if self.flage_run:
                ...
                self.ui.textEdit_log.append('显微镜平台对焦停止')
                self.ui.textEdit_log.append('显微镜平台扫描停止')
                self.pushbutton()
                return
            # 初步位置
            img = None
    ...
    def start(self):
        ...
        self.Saver.DataProcessing = data.data_processing(ID, number_x, number_y, path_save + '\\' + ID + '.jpg', multiple)
        try:
            ...
            if focusing_method == '快速扫描二':
                ...
                for point in points_4:
                    ...
                    zpos_4.append(zpos)
                self.updata_label_fcous_img.emit(image)
                for Point_XY in points_xy:
                    ...
                    timesave = get_time()
                    # 保存原图
                    self.Saver.enqueue(img, ID, zpos, a, Point_XY, timesave,
