...
class MyMainWindow(QMainWindow):
    ...
    def mouseDoubleClickEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if ID is not None:
                w, h = 5120, 5120
                self.pic1 = Image.new("RGB", size=(w, h))
                self.pic2 = Image.new("RGB", size=(w, h))
                code = Graph_.get_IDfromCode(ID + '_50x')
                ...
                if size[0] > 0 and size[1] > 0:
                    ...
                    for i in list_pic:
                        ...
                        image1 = image.resize((int(w / 2), int(h / 2)))
                        image2 = image.resize((int(w / 2), int(h / 2)))
                        bbox = node_info['Data']['bbox']
                        draw = ImageDraw.Draw(image2)
                        for point in bbox:
                            draw.rectangle((int(point[0] * (w / 2) - point[2] * (w / 4)),
                                            int(point[1] * (h / 2) - point[3] * (h / 4)),
                                            int(point[0] * (w / 2) + point[2] * (w / 4)),
                                            int(point[1] * (h / 2) + point[3] * (h / 4))), outline="green", width=10)

                        self.pic1.paste(image1, box=(i[1] * int(w / 2), i[2] * int(h / 2)))
                    ...
                    # 创建 MyDialog 对象并显示
                    self.MyQMainWindow = MyQMainWindow(qpixmap)
    ...
    def initialize(self):
        ...
        try:
            ...
            self.RUN = Run.Run(self.Device, self.ui, self.model)
            self.Device.Microscope.navigationController.home_z()
            while self.Device.Microscope.microcontroller.is_busy():
                time.sleep(0.005)
            self.Device.Microscope.navigationController.home_xy()
            while self.Device.Microscope.microcontroller.is_busy():
                time.sleep(0.005)
            self.Device.Microscope.navigationController.xyPos.connect(self.updateLabelXYpos)
            self.Device.Microscope.navigationController.zPos.connect(self.updateLabelZpos)
    ...
    def eject(self):
        ...
        try:
            ...
            while self.Device.Microscope.microcontroller.is_busy():
                time.sleep(0.005)
            self.Device.Microscope.navigationController.move_y_to(58)
            while self.Device.Microscope.microcontroller.is_busy():
                time.sleep(0.005)
    ...
    def live_20x(self):
        ...
        while self.Device.Microscope.microcontroller.is_busy():
            time.sleep(0.005)
        self.Device.Microscope.navigationController.move_z_to(5.160)
        while self.Device.Microscope.microcontroller.is_busy():
            time.sleep(0.005)
        self.flage_50x = False
        self.flage_20x = True
