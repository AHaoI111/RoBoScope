...
from PySide6.QtWidgets import QMainWindow, QGraphicsScene, QGraphicsPixmapItem, QMessageBox
...
def create_qimage_from_cvimg(cvimg, format_):
    ...
    # Create a QImage object using the data of the OpenCV image, specifying the width, height, bytes per line, and format
    return QtGui.QImage(
...
class MyMainWindow(QMainWindow):
    ...
    def initialize(self):
        ...
        if self.config_info['Device']['microscope']:
            ...
            if flag:
                ...
                QMessageBox.warning(None, "设备加载失败！", "设备加载失败！请检查设备连线")
                return
            # 进行设备自检
            ...
            if flag:
                ...
                QMessageBox.warning(None, "设备自检失败！", "设备自检失败！请检查设备连线")
                return
            # 初始化任务对象，并设置相关事件处理函数
            # 初始化任务
            ...
            QMessageBox.warning(None, "设备自检警告！", "上一次软件恶意闪退，请检查设备上有无玻片")
    ...
    def finish(self, code, img, multiple):
        ...
        # 创建一个文件处理器，将日志写入到文件中
        file_handler = logging.FileHandler(str('ID') + '.log')
        ...
        logger.info('玻片' + code + '已扫描')
        # 获取拼接图
        node = self.Saver.DataProcessing.G.get_IDfromCode(code + '_' + multiple)
        node_info = self.Saver.DataProcessing.G.get_node_info(node)
        height, width, channel = img.shape
        qImg = QImage(img.data, width, height, 3 * width, QImage.Format_RGB888)
        # 将QImage转换为QPixmap
        pixmap = QPixmap.fromImage(qImg)
        self.pixmap_scale2 = pixmap.scaled(self.ui.graphicsView.size(), QtCore.Qt.IgnoreAspectRatio,
        ...
        list_window = node_info['Data']['细胞推荐视野']
        list_window_bacteria = node_info['Data']['细菌推荐视野']
        view_number = list_hw
        ...
        self.ui.graphicsView.show()
        self.scene_puzzle.clear()
        pixmap_item = QGraphicsPixmapItem(self.pixmap_scale2)
        self.scene_puzzle.addItem(pixmap_item)
        ...
        mean_wbc = node_info['Data']['白细胞平均数']
        median_wbc = node_info['Data']['白细胞中位数']
        mean_ec = node_info['Data']['上皮细胞平均数']
        median_ec = node_info['Data']['上皮细胞中位数']
        mean_bacteria = node_info['Data']['细菌平均数']
        median_bacteria = node_info['Data']['细菌中位数']

        result_text = f'玻片ID：{code}\n' \
                      f'白细胞平均数：{mean_wbc}     白细胞中位数：{median_wbc}\n\n' \
                      f'上皮细胞平均数：{mean_ec}     上皮细胞中位数：{median_ec}\n\n' \
                      f'细菌平均数：{mean_bacteria}     细菌中位数：{median_bacteria}\n\n'

        if (mean_ec <= self.number_mean_ec and mean_wbc >= self.number_mean_wbc) or \
                (median_ec <= self.number_median_ec and median_wbc >= self.number_median_wbc):
            result_status = '该玻片合格'
        else:
            result_status = '该玻片不合格'

        self.ui.label_result_AI.setText(result_text + result_status)
