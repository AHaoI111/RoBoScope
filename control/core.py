...
class ImageDisplay(QObject):
    ...
    def __init__(self):
        QObject.__init__(self)
        self.queue = Queue(12)  # max 10 items in the queue
        self.image_lock = Lock()
        self.stop_signal_received = False
        self.thread = Thread(target=self.process_queue)
        self.thread.start()
    ...
    def process_queue(self):
        while True:
            ...
            # process the queue
            try:
                [image, frame_ID, timestamp] = self.queue.get(timeout=0.002)
                self.image_lock.acquire(True)
...
class Configuration:
    def __init__(self, mode_id=None, name=None, camera_sn=None, exposure_time=None, analog_gain=None,
                 illumination_source=None, illumination_intensity=None):
        self.id = mode_id
        self.name = name
                    ...
                    if RUN_CUSTOM_MULTIPOINT and "multipoint_custom_script_entry" in globals():
                        ...
                        for k in range(self.NZ):
                            ...
                            file_ID = coordiante_name + str(i) + '_' + str(
                                j if self.x_scan_direction == 1 else self.NX - 1 - j) + '_' + str(k)
