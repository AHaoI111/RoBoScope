...
class Camera(object):
    ...
    def open(self, index=0):
        ...
        # temporary
        self.camera.AcquisitionFrameRate.set(1500)
    ...
    def enable_callback(self):
        if self.callback_is_enabled == False:
            ...
            # enable callback
            user_param = None
            self.camera.data_stream[0].register_capture_callback( self._on_frame_callback)
    ...
    def set_exposure_time(self, exposure_time):
        ...
        if use_strobe == False or self.is_global_shutter:
            ...
            camera_exposure_time = self.exposure_delay_us + self.exposure_time * 1000 + self.row_period_us * self.pixel_size_byte * (
                        self.row_numbers - 1)   # add an additional 500 us so that the illumination can fully turn off before rows start to end exposure
    ...
    def _on_frame_callback(self, raw_image):
