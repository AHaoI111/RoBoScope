...
class MyWindow(QMainWindow, QObject):
    ...
    def device(self):
        ...
        if self.motion is None:
            try:
                ...
                if self.motion.navigationController is None:
                    self.ui.log.append(formatted_time + '     ' + '! 显微镜系统未检测到 !')
                else:
                    # 信号
                    ...
                    self.ui.log.append(formatted_time + '     ' + '! 显微镜系统连接成功 !')
            except:
                self.ui.log.append(formatted_time + '     ' + '! 显微镜系统未检测到 !')
        else:
            self.ui.log.append(formatted_time + '     ' + '! 显微镜系统已经在线 !')
        if self.camera is None:
            if self.motion.microcontroller is not None:
                ...
                self.ui.log.append(formatted_time + '     ' + '! 由于显微镜系统不在线，不开启相机 !')
        else:
    ...
    def motion_only(self):
        ...
        while self.motion.microcontroller.is_busy():
            time.sleep(0.005)
        self.move = move.Move(self.camera, self.motion, self.Sketch_map, self.ui, self.model)
