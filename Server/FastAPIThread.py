...
class HTTPServer(threading.Thread, QObject):
    ...
    def __init__(self, ipadd, port):
        ...
        async def get_config_info():
            try:
                info = {}
                if self.Com2scope.config_info['Microscope']['sys']['当前系统'] == 'single':
                    key_to_remove = ['low', 'high']
                    updated_json = remove_specific_keys(self.Com2scope.config_info, key_to_remove)
                    info = {'sn': self.Com2scope.config_info['Device']['sn'],
                            "sys": self.Com2scope.config_info['Microscope']['sys']['当前系统'],
                            "MultipleSingle": self.Com2scope.config_info['Microscope']['single']['单镜头倍数'],
                            "MultipleLow": "None",
                            "MultipleHigh": "None",
                            "focus_mode": [
                                {
                ...
                elif self.Com2scope.config_info['Microscope']['sys']['当前系统'] == 'double':
                    key_to_remove = ['single']
                    updated_json = remove_specific_keys(self.Com2scope.config_info, key_to_remove)
                    info = {'sn': self.Com2scope.config_info['Device']['sn'],
                            "sys": self.Com2scope.config_info['Microscope']['sys']['当前系统'],
                            "MultipleSingle": "None",
                            "MultipleLow": self.Com2scope.config_info['Microscope']['low']['低倍倍数'],
                            "MultipleHigh": self.Com2scope.config_info['Microscope']['high']['高倍倍数'],
                            "focus_mode": [
                                {
                                    "id": 1,
                                    "name": "中心对焦",
        ...
        async def pre_scan_pic(info: PreScanPicRequest):
            try:
                Taskinfo = apply_task_info(self.Com2scope.config_info)
                Taskinfo['scanmode'] = False
                Taskinfo['scanmultiple'] = 'low'
                Taskinfo['region_w_low'] = int(2560 * Taskinfo['calibration_low']) + 1
                Taskinfo['region_h_low'] = int(2560 * Taskinfo['calibration_low']) + 1
                Taskinfo['pre_request_flag'] = True
                Taskinfo['task_id'] = info.guid
                Taskinfo['pre_scan_api'] = info.return_api
                Taskinfo['FocusMode_low'] = 1
                Taskinfo['recommend_view_source'] = None
                if self.Com2scope.config_info['Microscope']['sys']['当前系统'] == 'double':
                    self.Com2scope.send_scan_pic.emit(Taskinfo)
                return {
                    "result": "success",
                    "msg": "调用成功"}
            except Exception as e:
