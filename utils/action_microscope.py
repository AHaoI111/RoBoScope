...
class ActionMicroscope(QtCore.QObject):
    ...
    def read_config(self):
        ...
        led = self.config_info['Microscope']['led']
        ...
        if multiple_sys == '单镜头':
            return center_x_only, center_y_only, region_w, region_h, calibration_only, focusing_method, zpos_startonly, multiple, focu_number, focu_size, led, multiple_sys
        elif multiple_sys == '双镜头':
            if multiple == '20x':
                return center_x_20x, center_y_20x, region_w, region_h, calibration_20, focusing_method, zpos_start20x, multiple, focu_number, focu_size, led, multiple_sys
            elif multiple == '50x':
                return center_x_50x, center_y_50x, region_w, region_h, calibration_50, focusing_method, zpos_start50x, multiple, focu_number, focu_size, led, multiple_sys
    ...
    def set_left_light(self):
        ...
        self.Device.microcontroller.set_illumination(
            self.Device.configurationManager.configurations[0].illumination_source,
            self.Device.configurationManager.configurations[0].illumination_intensity)

    # 设置1号灯
    def set_right_light(self):
        self.Device.microcontroller.turn_off_illumination()
        self.Device.microcontroller.set_illumination(
            self.Device.configurationManager.configurations[1].illumination_source,
            self.Device.configurationManager.configurations[1].illumination_intensity)
    ...
    def set_led_matrix(self):
        ...
        self.Device.microcontroller.set_illumination_led_matrix(
            self.Device.configurationManager.configurations[0].illumination_source,
            r=(
                      self.Device.configurationManager.configurations[
                          0].illumination_intensity / 100) * LED_MATRIX_R_FACTOR,
            g=(
                      self.Device.configurationManager.configurations[
                          0].illumination_intensity / 100) * LED_MATRIX_G_FACTOR,
            b=(
                      self.Device.configurationManager.configurations[
                          0].illumination_intensity / 100) * LED_MATRIX_B_FACTOR)
    ...
    def Focus_on(self, multiple, zpos_start):
        ...
        try:
            ...
            zpos = self.Device.Microscope.navigationController.z_pos_mm  # 获取当前z轴位置
    ...
    def start(self, ID, IDall, slide_img):
        ...
        center_x, center_y, region_w, region_h, calibration, focusing_method, zpos_start, multiple, focus_numnber, focus_size, led, multiple_sys = self.read_config()
        # 移动到预设位置
        ...
        if len(ID) > 0:
            ...
            # 检查文件夹路径是否存在
            path_save = 'pic\\' + str(ID[0]) + '\\' + multiple
            ...
            # 检查文件夹路径是否存在
            path_save = 'pic\\' + str(IDall) + '\\' + multiple
            folder_path = os.path.join(path_save)
            if not os.path.exists(folder_path):
                # 如果路径不存在，则创建文件夹
                os.makedirs(folder_path)
        ...
        try:
            ...
            if focusing_method == '快速扫描二':
                ...
                for point in points_4:
                    ...
                    self.microscope_move_z_to(zpos_start)
                    zpos, image = self.Focus_on(multiple, zpos_start)
                    zpos_4.append(zpos)
                    self.updata_focus.emit(img)
                for Point_XY in points_xy:
                    ...
                    self.Saver.enqueue(img, UUID, zpos, a, Point_XY, timesave,
                                       ...
                                       multiple, code)
                    self.updata_Progress.emit(float(float(a) / len(points_xy)))
                    a = a + 1
                ...
                for Point_XY in points_xy:
                    ...
                    self.microscope_move_x_to(x_Start - (Point_XY[0] + 0.5) * w * calibration)
                    self.microscope_move_y_to(y_Start - (Point_XY[1] + 0.5) * h * calibration)
                    zpos, img = self.Focus_on(multiple, zpos_start - 0.005)
                    self.updata_point_draw.emit(Point_XY, region_w)
                    self.updata_focus.emit(img)
                    self.updata_puzzle.emit(a, Point_XY, img)
                    point_xy_real = [x_Start - (Point_XY[0] + 0.5) * w * calibration,
                ...
                for Point_XY in points_xy:
                    ...
                    if a == 1:
                        z_pos, img = self.Focus_on(multiple, zpos_start)
                        self.updata_focus.emit(img)
                        self.updata_puzzle.emit(a, Point_XY, img)
                        ...
                        z_pos, img = self.Focus_on(multiple, z_pos - 0.02)
                        self.updata_focus.emit(img)
                        self.updata_puzzle.emit(a, Point_XY, img)
                        self.updata_point_draw.emit(center_x, center_y, Point_XY, w * calibration, h * calibration,
                                                    region_w, region_h)
