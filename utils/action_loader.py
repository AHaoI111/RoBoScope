...
class ActionLoader(object):
    ...
    def update_slide_task(self, box, slide_id, flag):
        ...
        self.save_slide_task_file(self.slide_task)
    ...
    def save_slide_task_file(self, slide_task):
        try:
            # 确保 self.slide_task 是 NumPy 数组
            if not isinstance(slide_task, np.ndarray):
                raise TypeError("self.slide_task must be a NumPy array")

            # 保存文件
            np.save('slide_task.npy', slide_task)
        except Exception as e:
