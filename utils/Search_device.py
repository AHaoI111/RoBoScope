...
from Drives.microcontroller import Microcontroller
...
class device:
    ...
    def open_device(self):
        ...
        try:
            # 初始化显微镜控制器
            self.microcontroller = Microcontroller(COM=self.config['Microscope']['串口'])
            # 重置显微镜控制器
    ...
    def detection_device(self):
        ...
        try:
            ...
            if self.config['Device']['cameranumber'] == 1:
                ...
                if letters_only == 'FCU':
                    # 自动白平衡
                    if len(self.config['Camera']['wbone']) != 3:
                        self.camera.camera.BalanceWhiteAuto.set(2)
                    else:
                        self.camera.set_wb_ratios(self.config['Camera']['wbone'][0], self.config['Camera']['wbone'][1],
                                                  self.config['Camera']['wbone'][2])
                # 设置拍摄区域
                offset_x = (732 // 8) * 8
                offset_y = (238 // 8) * 8
                ...
                if letters_only == 'FCU':
                    # 自动白平衡
                    if len(self.config['Camera']['wb20x']) != 3:
                        self.camera1.camera.BalanceWhiteAuto.set(2)
                    else:
                        self.camera1.set_wb_ratios(self.config['Camera']['wb20x'][0], self.config['Camera']['wb20x'][1],
                                                   self.config['Camera']['wb20x'][2])
                # 设置拍摄区域
                offset_x = (732 // 8) * 8
                offset_y = (238 // 8) * 8
                ...
                if letters_only == 'FCU':
                    if len(self.config['Camera']['wb50x']) != 3:
                        self.camera2.camera.BalanceWhiteAuto.set(2)
                    else:
                        self.camera2.set_wb_ratios(self.config['Camera']['wb50x'][0], self.config['Camera']['wb50x'][1],
                                                   self.config['Camera']['wb50x'][2])
                ...
                if img is None:
                    return self.flag
            # 根据配置，初始化加载器
            if self.config['Device']['loaderflage'] == 'True':
                temp = self.Loader.set_loader_abs_y(0)
                if temp == 0:
                    pass
                elif temp == -1:
                    return self.flag
                temp = self.Loader.set_delivery_abs_x(0)
                if temp == 0:
                    pass
                elif temp == -1:
                    return self.flag
                temp = self.Loader.set_delivery_abs_z(0)
                if temp == 0:
                    pass
                elif temp == -1:
                    return self.flag
