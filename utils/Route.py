...
#         closest_index = i  # 初始化最近点为当前点
#         start_dist_closest_to_previous = np.sqrt((points[closest_index][0] - points[i - 1][0]) ** 2 +
#                                                  (points[closest_index][1] - points[i - 1][1]) ** 2)
#
#         # 计算当前点和后面所有点与前一个点的距离
#         for j in range(i + 1, n):
#             dist_current_to_previous = np.sqrt((points[j][0] - points[i - 1][0]) ** 2 +
#                                                (points[j][1] - points[i - 1][1]) ** 2)
#
#             # 找到最近的点
#             if dist_current_to_previous < start_dist_closest_to_previous:
#                 closest_index = j
#                 start_dist_closest_to_previous = dist_current_to_previous
#
#         # 如果找到的最近点与当前点不同，则进行交换
#         if closest_index != i:
#             points[i], points[closest_index] = points[closest_index], points[i]
#
#     return points
def sort_by_closeness(points):
    if not points:
