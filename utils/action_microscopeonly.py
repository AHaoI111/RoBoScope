                ...
                if Taskinfo['sys'] == 'single':
                    ...
                    if self.request is None:
                        ...
                        slide_id = self.request.create_slide_id(Taskinfo['task_id'],
                                                                '/' + year + '/' + month + '/' + day + '/' + Taskinfo['task_id'] + '/',
                                                                "col_row", img, number_y, number_x)
                        Taskinfo['flag_create_view'] = True
                    if slide_id is None:
                        self.updata_textEdit_log_microscope.emit('与服务器对接成功，扫描任务玻片ID为：' + str(slide_id))
                        # 清空点位
                        ...
                        # 检查文件夹路径是否存在
                        path_save = Taskinfo[
                                        'savepath'] + '/' + year + '/' + month + '/' + day + '/' + Taskinfo['task_id'] + '/' + str(
                            slide_id)
                        ...
                        self.updata_textEdit_log_microscope.emit('与服务器对接失败，扫描任务玻片ID为空')
                    ...
                    if Taskinfo['scanmode']:
                        ...
                        if self.request is None:
                            ...
                            slide_id = self.request.create_slide_id(Taskinfo['task_id'],
                                                                    '/' + year + '/' + month + '/' + day + '/' + Taskinfo['task_id'] + '/',
                                                                    "centerPoint", img, number_y, number_x)
                            Taskinfo['flag_create_view'] = True
                        if slide_id is not None:
                            self.updata_textEdit_log_microscope.emit(
                                '与服务器对接成功，扫描任务玻片ID为：' + str(slide_id))
                            # 清空点位
                            self.updata_point_clear.emit()
                            self.Saver.image_stitch_all = Image.new("RGB",
                                                                    (number_x * ImageStitchSize,
                            ...
                            year, month, day = get_time_2()
                            path_save = Taskinfo[
                                            'savepath'] + '/' + year + '/' + month + '/' + day + '/' + Taskinfo['task_id'] + '/' + slide_id
                            folder_path = os.path.join(path_save)
                            Taskinfo['flag_create_view'] = False
                            if not os.path.exists(folder_path):
                                # 如果路径不存在，则创建文件夹
